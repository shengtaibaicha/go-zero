...
import (
	...
	"go-zero/apps/file/rpc/tools/image"
	"go-zero/apps/models"
	"strings"
	"time"
)
...
func (l *UploadFileLogic) UploadFile(in *file.UploadReq) (*file.UploadResponse, error) {
	...
	// 存储原文件到minio
	response, err := up(l, in.Filename, in.File, in.Size, in.MimeType)
	if err != nil {
		return response, err
	}
	...
	if fileList[1] != "png" {
		...
		FileName := fileList[0] + "SE." + fileList[1]
...
func (l *UploadFileLogic) generateFileUrl(objectName string) string {
	...
	return l.svcCtx.Config.Minio.Endpoint + "/" + l.svcCtx.Config.Minio.Bucket + "/" + objectName
}
