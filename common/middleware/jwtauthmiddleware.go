...
func JwtAuthMiddleware(cfg JwtAuthConfig) func(http.HandlerFunc) http.HandlerFunc {
	...
	return func(next http.HandlerFunc) http.HandlerFunc {
		...
		return func(w http.ResponseWriter, r *http.Request) {
			...
			// 查询redis中是否有当前token,并且判断当前token与redis中是否匹配
			exists, err := cfg.RedisClient.Exists(claims.UserId)
			if exists == false || err != nil {
				logx.Errorf("JWT verify failed: %v, path: %s", err, r.URL.Path)
				httpx.WriteJson(w, http.StatusUnauthorized, map[string]string{
					"code":    "401",
					"message": "Redis not find token",
				})
				return
			}
			get, _ := cfg.RedisClient.Get(claims.UserId)
			if get != authHeader {
				logx.Errorf("JWT verify failed: %v, path: %s", err, r.URL.Path)
				httpx.WriteJson(w, http.StatusUnauthorized, map[string]string{
					"code":    "401",
					"message": "Redis token is not local token",
				})
				return
			}
